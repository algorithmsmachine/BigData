...
#specify the input log file and create an RDD containing the parsed log file data.
#
...
# ####Next, we use [`map(parseApacheLogLine)`]() to apply the parse function to each element (that is, a line from the log file) in the RDD and turn each line into a pair [`Row` object](http://spark.apache.org/docs/latest/api/python/pyspark.sql.html#pyspark.sql.Row).
